...
local function Dist2D(vec1, vec2)
  if vec1 and vec2 then
    local pos1 = (vec1.x and vec1 or (vec1.GetAbsOrigin and vec1.GetAbsOrigin() or 0));    
    local pos2 = (vec2.x and vec2 or (vec2.GetAbsOrigin and vec2.GetAbsOrigin() or 0));
...
function HitRunHeros.OnUpdate()
  if localHero and isUiEnabled1.GetValue() then
    ...
    if KeyBindOrbwalk.IsKeyDown() then
      ...
      table.sort(enemies, function(a, b) return a.GetAbs():Distance(localHero.GetAbsOrigin()) < b.GetAbsOrigin():Distance(localHero.GetAbsOrigin()) end);
