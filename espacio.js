		...
		if (KeyBindOrder.IsKeyDown()) {
			...
			if (target) {
				...
				if (unit && unit.IsAlive() && !unit.IsDormant()) {
					...
					myPlayer.PrepareUnitOrders(Enum.UnitOrder.DOTA_UNIT_ORDER_MOVE_TO_POSITION, null, blockingPos, null, Enum.PlayerOrderIssuer.DOTA_ORDER_ISSUER_PASSED_UNIT_ONLY, unit, false, true, 0.5);
				    //}
				}
			    });
			}
